...
const effectAll = func => iterable => {for (const x of iterable)
        func(x)};
const imageLoaded = (elm, image, gridBasis) => {
    ...
    if (elm instanceof HTMLElement) {
        ...
        elm.style.flexBasis = gridBasis * ratio + 'px'; // We'll then let each element grow dependent on it's width/height ratio.
        elm.style.flexGrow = ratio.toString(); // We're ready to show the photo to the public.
...
const buildGrid = grid => {
    ...
    effectAll(imageRatio(gridBasis))(grid.children); // The fake elements are used to make the last few images get decent sized.
...
addCss();
document.addEventListener('DOMContentLoaded', () => {findGrids()}, false);
